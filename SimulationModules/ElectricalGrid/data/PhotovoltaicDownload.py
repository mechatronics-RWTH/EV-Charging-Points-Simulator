...
EMAIL = "max.fassbender84@gmail.com"
...
def main():
    input_data = {
        'attributes': 'air_temperature,clearsky_dhi,clearsky_dni,clearsky_ghi,dhi,dni,ghi',
        'interval': '15',
    ...
    for name in ['2018']:
        ...
        for id, location_ids in enumerate(POINTS):
            ...
            logger.info(f'Making request for point group {id + 1} of {len(POINTS)}...')
            ...
            if '.csv' in BASE_URL:
                ...
                logger.info(f'Response data (you should replace this logger.info statement with your processing): {data}')
                ...
                logger.info(data['outputs']['message'])
                logger.info(f"Data can be downloaded from this url when ready: {download_url}")
...
def get_response_json_and_handle_errors(response: requests.Response) -> dict:
    ...
    if response.status_code != 200:
        ...
        logger.info(f"The response body: {response.text}")
        exit(1)
    ...
    if len(response_json['errors']) > 0:
        ...
        logger.info(f"The request errored out, here are the errors: {errors}")
        exit(1)
