...
from SimulationModules.ElectricitiyCost.ElectricyPrice import InterfacePriceTable
...
class RawEnvSpaceManager():
    ...
    def __init__(self, config: dict,
        ...
        self.read_config(config, self.parking_area.area_size)
        
    def read_config(self, config: dict, area_size: int):

        self.area_size=area_size

        self.price_table: InterfacePriceTable = config["price_table"]
        settings = config["settings"]
        self.step_time=settings.step_time
        self.settings = None if settings is None else settings
        self.start_time=settings.start_datetime
        self.sim_duration=settings.sim_duration
        ...
        self.horizon = 1
        if not "include_future_price" in config:
    ...
    def write_observations(self, 
        ...
        """
        passed_seconds=[(current_date_time-self.start_time).seconds]

        fields: List[Field] =self.parking_area.parking_area_fields

        grid_power=[self.local_grid.calculate_connection_point_load(current_date_time).get_in_w().value]
        building_power=[self.local_grid.calculate_building_contribution(current_date_time).get_in_w().value]
        pred_building_power=self.local_grid.get_building_power_future(date_time=current_date_time, horizon = horizon, step_time = timedelta(seconds=900))
        peak_grid_power=[self.local_grid.peak_grid_consumption.get_in_w().value]
        ...
        pv_power=[self.local_grid.calculate_pv_contribution(current_date_time).get_in_w().value]
        pred_pv_power=self.local_grid.get_pv_power_future(date_time=current_date_time, horizon = horizon, step_time = timedelta(seconds=900))
        ...
        #     #in here, we just use the sessions, where the cs is not a gini:
        #     if isinstance(session.charging_station, ChargingStation):
        #         index=session.field_index
        #         cs_charging_power[index]=session.actual_charging_power.get_in_w().value
        #         cs_charging_limits[index]=session.charging_station.get_maximum_cs_charging_power().get_in_w().value
        ...
        stat_batt_pwr_actual = [self.local_grid.stationary_battery.actual_grid_power.get_in_w().value] if self.local_grid.stationary_battery is not None else None
    ...
    def get_user_requests(self, fields):
        ...
        for i, field in enumerate(fields):
            request=self.charging_session_manager.get_request_object_by_field_index(i)
            if request is not None:
                if request.state == Request_state.REQUESTED:
                    user_requests[i]=1
                elif request.state == Request_state.CONFIRMED:
                    user_requests[i]=2
                elif request.state == Request_state.DENIED:
                    user_requests[i]=3
                elif request.state == Request_state.CHARGING:
                    user_requests[i]=4
                elif request.state == Request_state.SATISFIED:
                    user_requests[i]=5
            if field.has_parked_vehicle():
                if field.vehicle_parked.status == EV_modes.INTERRUPTING:
                    user_requests[i]=5 
    ...
    def get_gini_related_data(self):
        ...
        for i, gini in enumerate(self.parking_area.ginis):
            ...
            field_indices_ginis[i]=gini.field_index
    ...
    def get_charger_related_data(self, fields):
        ...
        fields_with_chargers: List[Field] = [field for field in fields if field.has_charging_station()]
        for field in fields_with_chargers:
            index=field.index
            cs_charging_power[index]=field.charger.get_power_contribution().get_in_w().value*(-1)
            cs_charging_limits[index]=field.charger.get_maximum_grid_power().get_in_w().value 
    ...
    def reset_obs(self, price_prediction: list):
        ...
        observation_raw={
            "grid_power": [0],
            "building_power": [0],
            "pred_building_power": np.array(self.pred_building_power),
            "peak_grid_power": [0],
            "price_table": price_prediction,
            "pv_power": [0],
            "pred_pv_power": self.pred_building_power,
