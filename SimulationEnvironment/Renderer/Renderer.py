...
from SimulationModules.ChargingSession.Request import Request_state
...
class PygameRenderer(Renderer):
    ...
    def save_video(self):
        ...
        print(f"Video saved at {self.video_creator.fullfilename}")
    ...
    def _render_power_consumers(self):
        ...
        self.pv_consumers_rendering = ObjectToRender(name = "pv_consumers",
                                                ...
                                                logo = logo_PV)
        try:
            print(self.render_environment.observation_raw["soc_stat_battery"])
    ...
    def update_stationary_storage(self):
        print(self.stationary_storage.soc)
        if self.render_environment.observation_raw["soc_stat_battery"] is not None:
    ...
    def update_time(self):
        ...
        text = font.render(f"{self.render_environment.current_time}", True, (0, 0, 0))
    ...
    def _create_gini_icons(self):
        ...
        fields_with_gini = self.render_environment.parking_area.get_fields_with_gini()
        for field in fields_with_gini:
            self.x=self.parking_area_field_width*field.position[0]
            self.y=self.SUPERMARKET_SCREEN_HEIGTH+ self.parking_area_field_heigth*field.position[1]
            gini = self.render_environment.parking_area.get_gini_by_field_index(field.index)    
            soc = gini.get_soc()
    ...
    def _apply_shift(self,field: ParkingSpot, shift_factor: numbers.Number=0.7):
        if field.has_parked_vehicle() or field.has_charging_station():
            ...
            if field.position[0]== 0: # if the vehicle is parked on the very left side
                ...
                field_to_the_left = self.render_environment.parking_area._get_field_by_position([field.position[0]-1,field.position[1]])
    ...
    def _show_request_status(self, field: Field):
        ...
        if not field.has_parked_vehicle():
            return
        field_request= self.render_environment.charging_session_manager.get_request_object_by_field_index(field.index)
    ...
    def _assign_icons(self):
        self.scaled_images={
            "parking_spot_logo" : pygame.transform.scale(ParkingSpot.pygame_logo,self.parking_area_field_size),
            "charging_spot_logo" : pygame.transform.scale(GiniChargingSpot.pygame_logo,self.parking_area_field_size),
            "obstacle_logo" : pygame.transform.scale(Obstacle.pygame_logo,self.parking_area_field_size),
            "path_logo" : pygame.transform.scale(ParkingPath.pygame_logo,self.parking_area_field_size),
            "gini_logo" : pygame.transform.scale(GINI.pygame_logo,self.parking_area_field_size),
            "ev_logo" : pygame.transform.scale(EV.pygame_logo,self.parking_area_field_size),
