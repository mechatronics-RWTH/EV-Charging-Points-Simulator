from pyomo.environ import ConcreteModel, Var, NonNegativeReals, Reals, Binary, Objective, maximize, Constraint, Param, SolverFactory, AbstractModel, value
...
class Pyomo_Optimization_Model(ConcreteModel, InterfaceOptimizationModel):
    ...
    def __init__(self,
        ...
        super(Pyomo_Optimization_Model, self).__init__()
        self.prediction_horizon = range(length_prediction_horizon)     
        self.parking_fields_indices = range(num_parking_fields)
        self.availability_horizon_matrix = availability_horizon_matrix
    ...
    def setup_variables(self):
        ...
        self.z_charger_occupied = Var(self.prediction_horizon, self.ROBOTS, self.CHARGERS, domain=Binary, initialize=0)  # Binary for robot charging at a charger
        self.z_parking_spot = Var(self.prediction_horizon, self.ROBOTS, self.parking_fields_indices, domain=Binary, initialize=0)  # Binary for robot charging an EV
    ...
    def setup_fixed_parameters(self, 
                               config: MpcConfig):
        # fixed parameters
        self.c_selling = Param(initialize=config.selling_cost_in_euro_per_kwh)  # Selling cost of the robot
        self.delta_t_in_h = Param(initialize=config.time_step_size.total_seconds()/3600, mutable=True)  # Time step
        self.max_charger_power = Param(initialize=config.max_charger_power, mutable = True)  # Maximum power from the grid
        self.max_power_robot = Param(self.ROBOTS, initialize=50, mutable=True)  # Maximum power of the robot
        self.E_total_robot = Param(self.ROBOTS, initialize=35, mutable=True)  # Total energy of the robot
        self.moving_penalty_parameter = Param(initialize=config.moving_penalty)  # Selling cost of the robot
        self.grid_balance_pen_per_kwH = Param(initialize=1)  # Penalty for deviation from the end of the horizon in Euro/kWh
        if config.use_slack_weight_end_of_horizon:
            self.use_slack_weight_end_of_horizon = True
            val = self.c_selling.value*0.5
        else:
            self.use_slack_weight_end_of_horizon = False
            val = 0
        self.penalty_E_robot_end_horizon_deviation = Param(initialize= val, mutable=True)  # Penalty for deviation from the end of the horizon in Euro/kWh
    ...
    def one_step_revenue_grid(self, m, t):
        selling_reward = self.c_selling*sum(self.P_robot_discharge[t,:])*self.delta_t_in_h
        buying_cost = self.P_price[t]*self.P_grid_draw[t]*self.delta_t_in_h
        slack_grid = self.slack_grid[t]*self.delta_t_in_h*self.grid_balance_pen_per_kwH
        return  selling_reward - buying_cost - slack_grid
    ...
    def revenue_rule(self,m):
        grid_reward = sum(self.one_step_revenue_grid(m, t) for t in self.prediction_horizon)
        return grid_reward -self.E_robot_not_fully_charged_penalty(m) - self.one_step_moving_penalty()
    ...
    def show_current_values(self, model_objects_to_print: List[Union[Var, Param, Constraint]] = None):
        if self.verbose:
            if model_objects_to_print is None:
                model_objects_to_print = [self.E_cur_ev]#self.P_building, self.P_PV, self.P_price]
