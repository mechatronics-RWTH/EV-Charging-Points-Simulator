...
import pathlib
from SimulationModules.ElectricalGrid.PhotovoltaicArray import read_PV_data, PhotovoltaicArray,MAX_POWER_FROM_TABLE
from SimulationModules.ChargingStation.ChargingStation import ChargingStation
from SimulationModules.ElectricalGrid.Building import Building, read_standard_load_data
...
def test_read_pv_data():
    ...
    #logger.info(time)
    #logger.info(PV_Power)
    #logger.info(PV_Power[5])
...
def test_pv_power_prediction():
    ...
    local_grid.connected_consumers.append(PhotovoltaicArray(name="pv_1",
                                                     starttime=datetime(year=2020,month=5, day=1, hour=0, minute=10), 
                                                     end_time=datetime(year=2020,month=6, day=1, hour=0, minute=10),
                                                     step_time=timedelta(hours=1),
                                                     max_pv_power=PowerType(power_in_w=20, unit=PowerTypeUnit.KW))
                                                     )
    preds=local_grid.get_pv_power_future(date_time=datetime(year=2020,month=5, day=5, hour=10, minute=10), horizon=5, step_time=timedelta(hours=1))
...
def test_building_power_prediction():
    ...
    local_grid.connected_consumers.append(Building(name="Supermarkt",
                                                    yearly_consumption=EnergyType(1000, unit=EnergyTypeUnit.KWH),
                                                     start_time=datetime(year=2020,month=5, day=1), 
                                                     end_time=datetime(year=2020,month=6, day=1),
                                                     step_time=timedelta(hours=1))
                                                     )
...
def test_sign_convention_for_stationary_storage_with_PV():
    ...
    stationary_storages.set_actual_consumer_charging_power(set_power_value)
    PV_Array = PhotovoltaicArray("PV1")
    date_time=datetime(year=2024, month=6, day=13, hour=12, minute=0)
